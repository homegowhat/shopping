...
public class Upload4w extends HttpServlet {
...
//		System.out.println("type, value = " + request.getParameter("type"));
		...
		String userPicPath = request.getServletContext().getRealPath("/WEB-INF/PIC/USER_PIC");
		String newsPic = request.getServletContext().getRealPath("/WEB-INF/PIC/"+type);
		File newsPicFile = new File(newsPic);
...
//						System.out.println("7");
						newFileName = user.getLoginName();
						filePath = userPicPath;
//					} else if (type != null && type.equalsIgnoreCase("news") && messageId != null) {
...
////						messageService.insertNewsImageById(newFileName, messageId);
//						newFileName = pictureService.addPicture(httpRequest, websiteId, String.valueOf(messageId), type.toUpperCase(), 0, user.getId());
//					} else if (type != null && type.equalsIgnoreCase("slide")) {
////						newFileName = UUID.randomUUID().toString().replace("-", "");
//						filePath = newsPic;
//						IPictureService pictureService = (IPictureService) getBean("pictureService", request.getServletContext());
////						messageService.insertNewsImageById(newFileName, messageId);
//						newFileName = pictureService.addPicture(httpRequest, websiteId, "0", type.toUpperCase(), 0, user.getId());
					} else {
...
//						System.out.println("8-2");
						//						messageService.insertNewsImageById(newFileName, messageId);
						newFileName = pictureService.addPicture(httpRequest, null, messageId!=null?String.valueOf(messageId):null, type.toUpperCase(), 0, user.getId());
